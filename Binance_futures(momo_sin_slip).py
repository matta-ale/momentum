...
def open_long_sin_slip(precio_input,activo,cantidad,digits): #compra en futuros   
    client.futures_change_leverage(symbol=activo, contractType = 'PERPETUAL', leverage=1)
...
def open_short_sin_slip(precio_input,activo,cantidad,digits): #short en futuros   
    client.futures_change_leverage(symbol=activo, contractType = 'PERPETUAL', leverage=1)
    cant_ajust = truncate(cantidad/float(precio_input),digits)
    cant_cripto_aper = cant_ajust
    startTime = time.time()
